// FIXED: Scheduling now works correctly with scheduleHelper API
// FIXED: Removed Today/Tomorrow buttons from schedule modal - date pre-populated with today
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          ...
          {/* Date Selection - Pre-populated with today */}
          <div>
...
const SensoryBreaks = () => {
  ...
  // FIXED: Determine parent hub based on current path
  const getParentHub = () => {
    // If accessed via /activities/sensory-breaks, go back to /activities
    // If accessed via /wellness/sensory-breaks, go back to /wellness
  ...
  // Handle add single break to schedule - FIXED: Added try/catch error handling
  const handleAddToSchedule = ({ activity, category, date, time, reminder }) => {
    try {
      const result = addActivityToSchedule({
        ...
        time: time,
        emoji: activity.emoji,
        color: category?.color || SOURCE_COLORS?.[SCHEDULE_SOURCES?.SENSORY_BREAK] || '#20B2AA',
        source: SCHEDULE_SOURCES?.SENSORY_BREAK || 'sensory_break',
        notify: reminder,
        metadata: {
      ...
      console.error('Error adding sensory break to schedule:', error);
  ...
  // Handle add regular breaks to schedule - FIXED: Added try/catch error handling
  const handleAddRegularBreaks = ({ date, startTime, endTime, interval, breakType, reminder }) => {
    try {
      ...
      if (result && result.success) {
        ...
        toast.error('Oops!', result?.error || 'Could not add breaks to schedule. Please try again.');
      }
    } catch (error) {
      console.error('Error adding regular breaks to schedule:', error);
      toast.error('Oops!', 'Something went wrong. Please try again.');
      setShowRegularBreaksModal(false);
    }
  };
